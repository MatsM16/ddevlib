export class SyntaxCompiler {
    ...
    static compiler(syntax) {
        ...
        if (syntax === "HTML") {
            const css = SyntaxCompiler.compiler("CSS");
            const js = SyntaxCompiler.compiler("JS");
            builder
                .symbol("css", /(?<=>)[^<]+(?=<\/\s*style\s*>)/gs, token => css(token.value))
                .symbol("css", /(?<=style=")(.|\n)*?(?<!\\)(?="[^<]*>)/gs, token => css(token.value))
                .symbol("js", /(?<=>)[^<]+(?=<\/\s*script\s*>)/gs, token => js(token.value));
        }
...
export class SyntaxCompilerJob {
    ...
    static _findTextTokens(source, tokens) {
        ...
        const addToken = (value) => leftovers.push({
            index: lastEnd,
            value: value,
            symbol: SyntaxCompilerJob.textSymbol()
        });
        const leftovers = [];
        while (tokenExists()) {
    ...
    static _compose(tokens) {
        ...
        for (const token of tokens) {
            ...
            const end = `</span>`;
            const text = token.symbol.composeInner ? token.symbol.composeInner(token) : this.defaultCompose(token);
            html += start + text + end;
        }
        return html;
    }
    static defaultCompose(token) {
        return SyntaxCompilerJob.escape(token.value);
    }
    static textSymbol() {
        return {
            name: "text",
...
//# sourceMappingURL=SyntaxCompiler.1.js.map