...
import { QueryBuilder } from "./Queries.js";
...
export class QDatabase {
    ...
    insert(table) {
        return QueryBuilder.buildInsert(query => new Promise((resolve, reject) => {
    ...
    delete(table) {
        return QueryBuilder.buildDelete(query => new Promise((resolve, reject) => {
            const { name, primary, primaryType } = this.getTable(table);
            const store = this.database.store(name, "readwrite");
            if (query.keys) {
    ...
    join(query) {
        ...
        return (main) => {
            return new Promise((resolve, reject) => {
                const otherJoin = query.join ? this.join(query.join) : ((i) => Promise.resolve(i));
                if (query.mode === "any") {
                    let match = false;
                    store.traverse(({ item }) => otherJoin(item)
    ...
    select(table) {
        return QueryBuilder.buildSelect(query => new Promise((resolve, reject) => {
            const { name, primary, primaryType } = this.getTable(table);
            ...
            const join = query.join ? this.join(query.join) : (i) => Promise.resolve(i);
            const traverse = (handle) => {
                let index = 0;
                let matchCount = 0;
                return store.traverse(({ item }) => join(item).then(mutate).then(final => {
                    if (matchCount >= limit)
                        return false;
            ...
            if (query.mode === "key") {
                ...
                let match = null;
                traverse(item => { match = item; if (item)
                    return false; })
    ...
    update(table) {
        return QueryBuilder.buildUpdate(query => new Promise((resolve, reject) => {
            ...
            if (query.keys) {
                ...
                const key = query.key;
                store.get(query.key)
                    .then(function (item) { if (condition(item)) {
                    updater(item);
